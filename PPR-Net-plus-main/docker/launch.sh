...
if [[ $have_image -eq 1 ]]; then
  ...
  echo "Building Image!"
  sudo docker build -f $NAME.Dockerfile -t $NAME .
fi
...
if [[ $have_container -eq 1 ]]; then
  ...
  echo "sudo docker run -it --gpus all --rm -v $(cd ../ && pwd):/ppr-net --network="host" $NAME bash"
  sudo docker run -it --gpus all --rm -v $(cd ../ && pwd):/ppr-net --network="host" $NAME bash
  exit 0
fi
